...
本登记覆盖当前 `web/snapshot.json` 暴露给离线运行时的全部知识切片；其中一部分尚未被三份预置画像的资源断言引用，但仍会被浏览器端检索器使用。页面中的“断言与溯源链”会显示同一条来源、定位和可点击的“打开原始来源”。
...
- 原始知识库保留了既有的 `verified: true` 历史记录（KB-004、KB-015、KB-016、KB-017、KB-018）；本次没有把任何新条目标为“已人工核实”。其中 KB-015、KB-016、KB-018 已因来源失效从正式 Demo 排除。`verified` 仅保留历史人工复核事实；对外展示的核实状态一律还要求 `demo_eligible: true`，因此失效切片不会显示为当前可核验来源。

## 可审计台账

当前静态 Demo 的范围不是全部 26 条知识库切片，而是 `web/snapshot.json` 中公开给
浏览器端检索器的 **20 条**。预置的 P-A、P-B、P-C 三个会话当前实际在资源断言中
引用其中 12 条：KB-001–KB-005、KB-009、KB-017、KB-019–KB-023；KB-006–KB-008、
KB-011–KB-013、KB-024、KB-026 虽未出现在预置资源中，仍会在离线互动生成时被检索，
