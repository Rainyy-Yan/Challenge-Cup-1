...
代码、本地单元测试、静态工作流检查、本地 Linux Docker 镜像构建和受限容器运行已经完成，仓库变量也已绑定
digest-pinned GHCR 引用。真实 GitHub 仓库中的授权变更评论、权限、并发和 Draft PR 端到端测试仍是未完成门禁；
在该门禁通过前，不得宣称授权变更通道已经上线或生产可用。
