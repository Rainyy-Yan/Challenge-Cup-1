...
机器人在 PR 创建、重新打开、转为 Ready 或推送新提交时运行，并由 `github-actions[bot]` 发布或更新一条中文审查评论。它只读取 PR 差异，不检出或执行待审分支代码。

## 配置

仓库必须设置 Actions Secret `MINIMAX_API_KEY`。不要将密钥写入源码、工作流变量、Issue、PR 或日志。
